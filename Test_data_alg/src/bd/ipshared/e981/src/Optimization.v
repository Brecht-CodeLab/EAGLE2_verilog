...
module Optimization (
	...
	reg [19:0] data_check_buf = 20'h7FFFF; //250 000 clks of stable period
	...
	always @(posedge clk)begin
	    if(~nrst)begin
	        ...
	        data_start_reg <= 0;
	        freq_new_reg <= freq;
	        data_check_buf <= 20'h7FFFF;
	    end
		else if(nrst && ~data_start)begin
		    if(~freq_optimum)begin
				//FREQ OPT ALG
		        data_go_reg <= 0;
		        data_start_reg <= 0;
	            if(freq_rdy == 1 && freq_set_up_down && (freq + delta_freq < 20'hEA60))begin
	                freq_new_reg <= freq + delta_freq;
	            end
	            else if (freq_rdy == 1 && ~freq_set_up_down && (freq - delta_freq > 20'h61A8)) begin
	                freq_new_reg <= freq - delta_freq;
	            end
	            else begin
	                freq_new_reg <= freq;
